...
const statusColors: Record<string, string> = {
    ...
    closed: 'bg-stone-100 text-stone-600 hover:bg-stone-200',
}
...
export default function StatusBadge({ leadId, currentStatus }: { leadId: string; currentStatus: string }) {
    ...
    return (
        <button
            ...
            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors cursor-pointer disabled:opacity-60 ${statusColors[currentStatus] ?? 'bg-stone-100 text-stone-600'}`}
        >
